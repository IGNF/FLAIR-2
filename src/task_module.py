...
class SegmentationTask(pl.LightningModule):
    ...
    # modified
    def forward(self, input_patch, input_spatch, input_dates, input_met):
        logits = self.model(input_patch, input_spatch, input_dates, input_met)
        return logits
    ...
    def step(self, batch):
        ...
        spatch = batch["spatch"]
        dates = batch["dates"]
        targets = batch["msk"].long()
        targets_sp = batch["smsk"].long()
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        if self.use_metadata == True:
            logits_utae, logits_unet = self.forward(batch["patch"], batch['spatch'], batch['dates'], batch["mtd"])
        else:
            logits_utae, logits_unet = self.forward(batch["patch"], batch['spatch'], batch['dates'], '')
