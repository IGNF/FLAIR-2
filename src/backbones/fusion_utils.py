...
#### IF FUSION_MODE IS CROPPED OR FULL    
class fmap_spatial_crash_mlps(nn.Module):
    """ 
    Deep or shallow MLPs to summarize spatial dim of featmap

    """
    def __init__(self, in_channels, out_channels_list, mlp_type='deep'):
        ...
        for out_ch_size in out_channels_list:
            ...
            if mlp_type == 'deep':
                ...
                if in_channels < out_ch_size: 
                    ...
                    self.mlps.append(nn.Sequential(*layers))  
            elif mlp_type == 'shallow':
                layers.extend([nn.Linear(in_channels, out_ch_size),])
                self.mlps.append(nn.Sequential(*layers))
