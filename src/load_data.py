...
def load_data (config: dict, val_percent=0.8): 
    ...
    print('','*'*100, sep='\n')
    for i,j in zip([dict_train, dict_val, dict_test],['TRAIN', 'VAL', 'TEST']):
        print('DATA {:<5s} --> AERIAL IMGS: {:<5}| SATELLITE IMGS: {:<5}| LABELS: {:<5}| MTD AERIAL: {:<5}'.format(j,len(i['PATH_IMG']),len(i['PATH_SP_DATA']),len(i['PATH_LABELS']), len(i['MTD_AERIAL'])))
    print('*'*100,'\n')   
